...
## 来点铺垫
Lua会先将脚本编译成字节码，然后由虚拟机执行。Lua closure的底层实现中，[`Proto.code`](/lua/function-implementation.md#结构原型)指向的就是存放字节码的数组。因此虚拟机的效率对Lua的运行至关重要。

### 栈和寄存器
在Lua 5.0版本之前，Lua采用的是**基于栈的虚拟机**架构；但从Lua 5.0开始，Lua虚拟机已重构为更高效的**基于寄存器的虚拟机**实现。在深入介绍Lua虚拟机的底层实现之前，要先理解这两种虚拟机的区别：
